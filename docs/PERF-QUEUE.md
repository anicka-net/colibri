...
### 2b. External Red Hat DSpark — measured incompatible with the int4 verifier

`RedHatAI/GLM-5.2-speculator.dspark` was tested on the second GB10 against
hidden states from this engine's actual GLM-5.2 int4 verifier.  The checkpoint
contains 3.807B BF16 parameters (7.1 GiB on disk), loaded all 64 tensors
without missing or unexpected keys, and ran a compiled one-anchor shape smoke
at a 38.6 ms warm median.  Under Transformers 5.6.2, the model-card
`AutoModel` snippet does not load with the checkpoint's current metadata; the
working loader was `SpeculatorModel` from the pre-v0.7 DSpark source revision
`dcb6f3c`.

The acceptance trace used 98 real chat-formatted tokens: 34 control/prompt
tokens plus a 64-token greedy answer generated by production Colibri.  CUDA
prefill dumped all residual layers; 56 valid assistant-token anchors were
scored.  Giving the checkpoint the more favorable of the two plausible layer
offset conventions produced:

- independent position-1 / position-2 accuracy: **67.9% / 39.3%**;
- cumulative width-2 acceptance: **67.9% first, 33.9% both**;
- width-2 accepted drafts per cycle: **1.018** (50.9% of proposals);
- mean accepted length across all seven positions: **1.214**.

Confidence gating did not rescue the economics.  At threshold 0.4, coverage
was 58.9% with 60.6% proposal acceptance.  Threshold 0.7 reached 83.3%
acceptance at only 10.7% coverage; threshold 0.8 was perfect on three samples
but covered 5.4%.  No operating point approached the precommitted roughly 70%
coverage plus near-94% width-2 acceptance gate.  The published checkpoint was
trained against GLM-5.2-FP8 hidden states; its confidence collapse on Colibri's
int4 residuals is consistent with verifier-representation drift.  Do not build
the remote hidden-state bridge for this checkpoint.
...
#### Measured dead end: caching prefill RoPE phases

The prefill kernel recomputes `powf`/`cosf`/`sinf` for every query head and
layer, which looked expensive by operation count.  A prototype cached one
FP32 phase table per `(position range, rope width, theta)` and used a
warp-per-row apply kernel.  It matched the legacy CUDA output within `1e-4`.

An isolated GB10 benchmark issued the exact GLM-5.2 query and K RoPE shapes
per layer (`Q = S*64`, `K = S`, `R = 64`), with five paired repetitions:

- At `S=23,606`, the median saving was **0.178 ms/layer**, or about 13 ms
  across 75 layers.
- At `S=2,701`, the median saving was **2.345 ms total** across all 75 layers.

The combined projection/RoPE profile bucket is therefore dominated by the
projection GEMMs.  The cache adds persistent device state and invalidation
logic for a gain far below measurement relevance, so the prototype was not
shipped.
...
append/prefill).  Dense MLP tensors are colocated with `kv_b`; snapshot/CPU
