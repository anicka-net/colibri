...
| `RSS_GUARD_GB` | resolved `RAM_GB`/auto budget | Linux measured-RSS emergency ceiling for non-mmap expert storage. Every 16 emitted tokens, the engine drains pilot I/O and lowers the LRU cap if current RSS exceeds the budget by 2% + 0.3 GB. Set `0` to disable. |
...
| `COLI_KV_DTYPE` | `fp16` | Device-shadow storage for latent, RoPE, and DSA-index KV rows: `fp32`, `fp16`, or scaled FP8 E4M3. FP8 keeps the host/persisted cache canonical FP32 and stores one FP32 `amax/448` scale per row and component. This setting takes precedence over legacy `COLI_KV_F16`; keep FP8 opt-in until the target hardware passes the long-context gate. |
| `COLI_KV_F16` | `1` | Legacy FP32/FP16 device-shadow switch, consulted only when `COLI_KV_DTYPE` is unset. |
| `COLI_DSA_DECODE_TCGATHER` | `1` | Gather selected decode KV rows once and use tensor-core score/context contractions. Supports row-INT4, row-INT8, and BF16 resident attention matrices; FP8 KV rows are dequantized to FP16 while gathering. `=0` retains the scalar selected-attention oracle for A/B testing. |
| `COLI_DSA_TCGATHER` | `1` | Equivalent selected-row tensor-core gather for DSA prefill phase B. `=0` retains the scalar prefill oracle. |
