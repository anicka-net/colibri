/* Loader-seam test for the named INT3 format: writes a real .safetensors file containing an
 ...
 * checks the byte-count/.qs-size format inference picks INT3 vs fmt=2 correctly
...
int main(void){
    ...
    CHECK(t3.fmt==COLI_TENSOR_INT3_GROUP);
    static float dq_load[O*I], dq_ref[O*I];
    deq4(&t3,dq_load);
    QT tr={.fmt=COLI_TENSOR_INT3_GROUP,.q4=q3,.s=s3,.O=O,.I=I};
    deq4(&tr,dq_ref);
