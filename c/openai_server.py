...
def anthropic_tools_compat(tools):
    result = []
...
class APIHandler(BaseHTTPRequestHandler):
        ...
        try:
            ...
            if path == "/health":
                ...
                if self._is_authed():
                    ...
                    payload["watchdog_active"] = self.server.watchdog_snapshot()
    ...
    def anthropic_count_tokens(self, body, request_id):
        ...
        normalized["tools"] = anthropic_tools_compat(body.get("tools"))
        thinking, effort = reasoning_settings(normalized, self.server.default_thinking)
        prompt = render_chat(normalized["messages"], thinking, effort,
    ...
    def _ollama_run(self, body, prompt, request_id, chat, tools, thinking):
        ...
        with self.server.watchdog_request(watchdog), \
            ...
            output = []
            if not stream:
                stats = engine_generate(self.server.engine, prompt, maximum, temperature, top_p,
    ...
    def anthropic_message(self, body, request_id):
        ...
        if not body.get("stream", False):
            ...
            if text or (not calls and not reasoning):
            ...
            if model.startswith("deepseek-"):
                usage.update(cache_read_input_tokens=0, cache_creation_input_tokens=0)
        ...
        def factory(write):
            ...
            def stop_current():
                if state["kind"] is not None:
                    if state["kind"] == "thinking":
                        event(write, "content_block_delta", {
            ...
            def start(kind, block=None):
                ...
                event(write, "content_block_start", {"type": "content_block_start",
                                                     ...
                                                     "content_block": block})
            state["start"] = start
            state["stop"] = stop_current
            if tools and not thinking:
                # Anthropic streams number the initial text block as zero even
                # when the model proceeds directly to a tool-use block.
                start("text")
            def reasoning(chunk):
                if state["kind"] != "thinking":
                    start("thinking")
        ...
        with self.server.watchdog_request(watchdog), \
            ...
            if not stream:
                ...
                if ARCH == "inkling":
                    ...
                    reasoning, text = split_reasoning(text, thinking)
