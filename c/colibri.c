...
typedef struct {
    ...
    float **cuda_Lc_scale,**cuda_Rc_scale;
    int *cuda_valid;
    float **cuda_Ic,**cuda_Ic_scale; int *cuda_ic_valid; /* ombra + scale FP8 per riga */
#endif
    int disk_nrec;
    char disk_path[2048];
...
static void expert_gate_up(float *g,float *u,const float *x,QT *wg,QT *wu,int S){
    if(!g_no_fused_pair&&!spec_pinned()&&S==1&&wg->fmt==2&&wu->fmt==2&&wg->I==wu->I&&wg->O==wu->O)
        ...
        matmul_i4_grouped(g,x,wg->q4,wg->s,S,wg->I,wg->O,wg->gs);
        matmul_i4_grouped(u,x,wu->q4,wu->s,S,wu->I,wu->O,wu->gs);
    }
...
static int g_cuda_router;
static double g_cuda_reserve_gb=2.0;
...
static int qt_cuda_upload(QT *t){
    ...
    if(t->gs>0)
        return coli_cuda_tensor_upload_g(&t->cuda,weights,t->s,t->fmt,t->I,t->O,
                                         t->cuda_device,t->gs);
...
static void cuda_stats_print(void){
    ...
    {uint64_t n=0,g=0,f=0,e=0;coli_cuda_nvfp4_stats(&n,&g,&f,&e);
      ...
      uint64_t gc=0,gp=0,gf=0;coli_cuda_nvfp4_grouped_stats(&gc,&gp,&gf);
      if(gc||gf)fprintf(stderr,"[CUDA] NVFP4 grouped: %llu launches | %llu problems | %llu fallbacks\n",
        (unsigned long long)gc,(unsigned long long)gp,(unsigned long long)gf);}
...
typedef struct {
    ...
    uint64_t dc_n[2],dc_direct_n[2];
    int64_t dc_bytes[2],dc_ns[2],dc_wall_ns[2],dc_wall_all_ns;
} ProfBase;
#ifdef COLI_CUDA
...
static void matmul_qt_ex(float *y, const float *x, QT *w, int S, int allow_idot){
    ...
    if(g_cuda_enabled && w->cuda_eligible && !w->cuda_failed && !omp_in_parallel()){
        ...
        if(coli_cuda_matmul(&w->cuda,y,x,weights,w->s,w->fmt,S,w->I,w->O,
                            w->cuda_device,w->gs)) return;
        w->cuda_failed=1;
    ...
    if(w->fmt==COLI_TENSOR_E8){ matmul_e8(y,x,w->q4,NULL,S,w->I,w->O); return; }   /* scales live in-block */
...
static void qt_alloc(QT *t, int O, int I, int bits){
    ...
    else if(bits==3){ t->fmt=COLI_TENSOR_INT3_GROUP; t->q4=qalloc((int64_t)O*i3_rowbytes(I));
...
static void qt_fill(QT *t, const float *w, int bits){
    ...
    else if(t->fmt==COLI_TENSOR_INT3_GROUP) pack_int3_g64(w, t->q4, t->s, t->O, t->I);
...
static int qt_resolve_fmt(const char *name, int O, int I, int64_t nb, int64_t ns, int *gs){
    ...
    if(ns==4 && nb==(int64_t)O*e8_rowbytes(I)){ *gs=0; return COLI_TENSOR_E8; }
    ...
    int fmt = (nb==exp_i8)?COLI_TENSOR_INT8_ROW
            : (nb==exp_i4)?COLI_TENSOR_INT4_ROW
            : (nb==exp_i2)?COLI_TENSOR_INT2_ROW
            : (nb==exp_i3)?COLI_TENSOR_INT3_GROUP : 0;
    if(!fmt){
    ...
    *gs=0;
    ...
    int64_t exp_scale = (fmt==4)? (int64_t)O*((I+*gs-1)/(*gs))
                      : (fmt==COLI_TENSOR_INT3_GROUP)? (int64_t)O*i3_groups(I) : (int64_t)O;   /* in FLOAT */
    if(ns != exp_scale*4){
...
static void qt_from_disk(Model *m, const char *name, int O, int I, int bits, int drop, QT *t){
    ...
    if(st_has(&m->S,sn)){
        ...
        else if(fmt==COLI_TENSOR_INT3_GROUP){ int64_t ng=i3_groups(I);   /* int3-g64: 24B/group weights + O*ng group scales */
            if(t->fmt!=COLI_TENSOR_INT3_GROUP||!t->q4){ t->fmt=COLI_TENSOR_INT3_GROUP; t->O=O; t->I=I; t->gs=0; t->q4=qalloc(nb); t->s=falloc((int64_t)O*ng); }
            st_read_raw(&m->S,name,t->q4,drop); }
        else if(fmt==COLI_TENSOR_E8){   /* E8/IQ3: everything in-block, .qs is the 4-byte tag */
            if(t->fmt!=COLI_TENSOR_E8||!t->q4){ t->fmt=COLI_TENSOR_E8; t->O=O; t->I=I; t->gs=0; t->q4=qalloc(nb); t->s=qsalloc(1); }
            st_read_raw(&m->S,name,t->q4,drop); }
...
static int expert_classify(Model *m, int layer, int eid){
    if(!m->elast_pre || !m->elast_pre[layer]) return DC_WARM;
    uint32_t last_pre=m->elast_pre[layer][eid];
    if(last_pre==0) return DC_COLD;
    uint32_t age=m->eaccess_clock_dc-last_pre;
    return age>g_direct_heat_ticks ? DC_COLD : DC_WARM;
}
...
static void qt_addrow(const QT *t, int row, float coef, float *acc){
    ...
    if(t->fmt==COLI_TENSOR_BF16){
        const uint16_t *w=t->bf16+(int64_t)row*I;
        for(int i=0;i<I;i++) acc[i]+=coef*coli_bf16_f32(w[i]);
        return;
    }
    ...
    if(t->fmt==COLI_TENSOR_INT3_GROUP){ const uint8_t *w=t->q4+(int64_t)row*i3_rowbytes(I);
...
static void qt_matvec_rows(const QT *t, int r0, int n, const float *x, float *y){
    ...
    for(int j=0;j<n;j++){ int row=r0+j; double a=0;
        ...
        else if(t->fmt==COLI_TENSOR_BF16){
            const uint16_t *w=t->bf16+(int64_t)row*I;
...
static size_t kv_esz(void){
    static size_t e=0;
    if(!e){int dtype=coli_cuda_kv_dtype();e=dtype==COLI_KV_FP8_E4M3?1:dtype==COLI_KV_FP16?2:4;}
    return e;
}
...
static int kv_dev_sync(Model *m, Layer *l, KVState *ks, int layer, int upto){
    Cfg *c=&m->c; int kvl=c->kv_lora, R=c->qk_rope, dev=l->kv_b.cuda_device;
    if(!ks||upto>ks->max_t) return 0;
    int fp8=coli_cuda_kv_dtype()==COLI_KV_FP8_E4M3;
    if(!ks->cuda_Lc[layer]||!ks->cuda_Rc[layer]||
       (fp8&&(!ks->cuda_Lc_scale[layer]||!ks->cuda_Rc_scale[layer]))){
        if(ks->cuda_Lc[layer]) coli_cuda_pipe_free(dev,ks->cuda_Lc[layer]);
        if(ks->cuda_Rc[layer]) coli_cuda_pipe_free(dev,ks->cuda_Rc[layer]);
        if(ks->cuda_Lc_scale[layer]) coli_cuda_pipe_free(dev,ks->cuda_Lc_scale[layer]);
        if(ks->cuda_Rc_scale[layer]) coli_cuda_pipe_free(dev,ks->cuda_Rc_scale[layer]);
        ks->cuda_Lc[layer]=NULL;ks->cuda_Rc[layer]=NULL;
        ks->cuda_Lc_scale[layer]=NULL;ks->cuda_Rc_scale[layer]=NULL;
        ...
        float *Ls=fp8?(float*)coli_cuda_pipe_alloc(dev,(size_t)ks->max_t*sizeof(float)):NULL;
        float *Rs=fp8?(float*)coli_cuda_pipe_alloc(dev,(size_t)ks->max_t*sizeof(float)):NULL;
        if(!Lc||!Rc||(fp8&&(!Ls||!Rs))){
            if(Lc) coli_cuda_pipe_free(dev,Lc);
            if(Rc) coli_cuda_pipe_free(dev,Rc);
            if(Ls) coli_cuda_pipe_free(dev,Ls);if(Rs) coli_cuda_pipe_free(dev,Rs);
            return 0;
        }
        ks->cuda_Lc[layer]=Lc;ks->cuda_Rc[layer]=Rc;
        ks->cuda_Lc_scale[layer]=Ls;ks->cuda_Rc_scale[layer]=Rs;
        ks->cuda_valid[layer]=0;
    }
    int v=ks->cuda_valid[layer];
    if(v<upto){
        if(!coli_cuda_pipe_upload_kv_rows(dev,ks->cuda_Lc[layer],ks->cuda_Lc_scale[layer],
            coli_kv_row(ks->Lc[layer],v,kvl),upto-v,kvl,v)||
           !coli_cuda_pipe_upload_kv_rows(dev,ks->cuda_Rc[layer],ks->cuda_Rc_scale[layer],
            coli_kv_row(ks->Rc[layer],v,R),upto-v,R,v)) return 0;
...
static int ic_dev_sync(Model *m, Layer *l, KVState *ks, int layer, int upto){
    ...
    int fp8=coli_cuda_kv_dtype()==COLI_KV_FP8_E4M3;
    if(!ks->cuda_Ic[layer]||(fp8&&!ks->cuda_Ic_scale[layer])){
        float *Ic=(float*)coli_cuda_pipe_alloc(dev,(size_t)ks->max_t*hd*kv_esz());
        if(!Ic) return 0;
        float *Is=fp8?(float*)coli_cuda_pipe_alloc(dev,(size_t)ks->max_t*sizeof(float)):NULL;
        if(fp8&&!Is){coli_cuda_pipe_free(dev,Ic);return 0;}
        ks->cuda_Ic[layer]=Ic;
        ks->cuda_Ic_scale[layer]=Is;
        ks->cuda_ic_valid[layer]=0;
    }
    int v=ks->cuda_ic_valid[layer];
    if(v<upto){
        if(!coli_cuda_pipe_upload_kv_rows(dev,ks->cuda_Ic[layer],ks->cuda_Ic_scale[layer],
            coli_kv_row(ks->Ic[layer],v,hd),upto-v,hd,v)) return 0;
...
static int attn_pipe_prefill(Model *m, Layer *l, int layer, const float *x, int x_is_dev,
    ...
    float *chost=NULL,*kvls=NULL,*kvrs=NULL; int ok=0;
    ...
    { int dev_kv = g_cuda_pipe && layer<c->n_layers && kv_dev_sync(m,l,m->kv,layer,pos_base);
      ...
      if(dev_kv){ ld_=(float*)((char*)m->kv->cuda_Lc[layer]+(size_t)st0*kvl*kv_esz());
                  ...
                  if(fp8){kvls=m->kv->cuda_Lc_scale[layer]+st0;kvrs=m->kv->cuda_Rc_scale[layer]+st0;} }
      else if(fp8) goto done; /* FP8 requires its persistent row-scale shadow. */
      else if(old>0){
      ...
      if(!coli_cuda_pipe_copy2d_kv_rows(dev,ld_,kvls,cd,kvl+R,S,kvl,old)) goto done;
      if(!coli_cuda_pipe_copy2d_kv_rows(dev,rd,kvrs,cd+kvl,kvl+R,S,R,old)) goto done;
    ...
    if(sel_need){
        ...
        if(c->idx_type[layer]){
            ...
            dsac.d_Ic_scale=m->kv->cuda_Ic_scale[layer];
...
sel_fail:
    ...
    if(out_dev && !sel_arg && coli_cuda_kv_dtype()!=COLI_KV_FP8_E4M3 && l->n_kv_b_shard>1 &&
...
#endif
    {
        ...
        if(sel_arg){
            ...
            if(gemm_pref){
                ...
                if(outd)
                    ok=coli_cuda_prefill_attn_gemm(l->kv_b.cuda,l->o.cuda,outd,qd,ld_,rd,kvls,kvrs,
                        S,H,c->qk_nope,R,c->v_head,kvl,T,c->attn_scale,
...
#ifdef COLI_CUDA
        if(ks->cuda_valid&&layer<c->n_layers&&ks->cuda_valid[layer]>pos)
            ks->cuda_valid[layer]=pos;               /* riga riscritta: l'ombra si accorcia */
#endif
            ...
            if(all_sel){
                ...
                for(int s=0;s<S&&cuda_core;s++){
                    ...
                    if(!coli_cuda_attention_project_sel(l->kv_b.cuda,l->o.cuda,
                            ...
                            m->kv->cuda_Lc_scale[layer],m->kv->cuda_Rc_scale[layer],
...
static void moe(Model *m, Layer *l, int layer, float *x, int S, float *out, int with_shared){
    ...
    int *group_pack_row=group_enabled?xalloc((size_t)group_stride*g_cuda_ndev*sizeof(int),"moe group_pack_row"):NULL;
    float *group_pack_weight=group_enabled?xalloc((size_t)group_stride*g_cuda_ndev*sizeof(float),"moe group_pack_weight"):NULL;
    ...
    if(!routed_remote) for(int base=0;base<nu;base+=64){
        ...
        for(int j=0;j<nb;j++){ int eid=uniq[base+j]; ESlot *e=use[j];
            ...
            if(e->d.fmt==COLI_TENSOR_E8) e8_rot_rows(gg,nr,I);   /* down input is per-expert — rotate here */
...
static int pipe_layer_cuda(Model *m, Layer *l, int li, float *x_dev, int S, int pos_base,
    ...
    if(sel_active){
        ...
        if(dsa_idx_layer){
            ...
            dsac.d_Ic_scale=m->kv->cuda_Ic_scale[li];
            dsac.nh=c->index_nh; dsac.hd=c->index_hd;
    ...
    if(S<=4 && li<c->n_layers &&
       ...
       kv_dev_sync(m,l,m->kv,li,pos_base)){
        ...
        if(cw1&&cw2&&coli_cuda_pipe_attn_chain_v2(dev,x_dev,nrm_d,nrm_host,
               ...
               m->kv->cuda_Lc_scale[li],m->kv->cuda_Rc_scale[li],
...
static void kv_alloc(Model *m, int max_t){
    ...
    KVState *k=m->kv;
#ifdef COLI_CUDA
    if(k->cuda_Lc) for(int i=0;i<c->n_layers;i++){
        ...
        if(k->cuda_Lc_scale&&k->cuda_Lc_scale[i]) coli_cuda_pipe_free(dev,k->cuda_Lc_scale[i]);
        if(k->cuda_Rc_scale&&k->cuda_Rc_scale[i]) coli_cuda_pipe_free(dev,k->cuda_Rc_scale[i]);
    }
    if(k->cuda_Ic) for(int i=0;i<c->n_layers;i++){
        int dev=m->L[i].kv_b.cuda_device;
        if(k->cuda_Ic[i]) coli_cuda_pipe_free(dev,k->cuda_Ic[i]);
        if(k->cuda_Ic_scale&&k->cuda_Ic_scale[i]) coli_cuda_pipe_free(dev,k->cuda_Ic_scale[i]);
    }
    free(k->cuda_Lc);free(k->cuda_Rc);free(k->cuda_valid);
    free(k->cuda_Lc_scale);free(k->cuda_Rc_scale);
    free(k->cuda_Ic);free(k->cuda_Ic_scale);free(k->cuda_ic_valid);
    k->cuda_Lc=calloc(c->n_layers,sizeof(float*));
    k->cuda_Rc=calloc(c->n_layers,sizeof(float*));
    k->cuda_Lc_scale=calloc(c->n_layers,sizeof(float*));
    k->cuda_Rc_scale=calloc(c->n_layers,sizeof(float*));
    k->cuda_valid=calloc(c->n_layers,sizeof(int));
    k->cuda_Ic=calloc(c->n_layers,sizeof(float*));
    k->cuda_Ic_scale=calloc(c->n_layers,sizeof(float*));
...
static void repin_pass(Model *m){ repin_pass_limit(m,m->c.n_layers); }
...
#ifdef COLI_CUDA
    if(k->cuda_Lc) for(int i=0;i<m->c.n_layers;i++){
        int dev=m->L[i].kv_b.cuda_device;
        if(k->cuda_Lc[i]) coli_cuda_pipe_free(dev,k->cuda_Lc[i]);
        if(k->cuda_Rc[i]) coli_cuda_pipe_free(dev,k->cuda_Rc[i]);
...
static void run_serve_mux(Model *m, const char *snap){
    ...
    GrDraft *grd=calloc(nctx,sizeof(*grd));
...
static double cuda_ic_shadow_bytes_for(int full,int index_hd,int max_ctx,int slots,
                                       int elem_bytes,int row_scale_bytes){
    return (double)slots*full*max_ctx*((double)index_hd*elem_bytes+row_scale_bytes);
}
...
static double cuda_ic_shadow_bytes(Model *m, int max_ctx){
    ...
    int slots=kv_slot_count(); if(slots<1||slots>16) slots=1;
    int dtype=coli_cuda_kv_dtype();
    return cuda_ic_shadow_bytes_for(full,m->c.index_hd,max_ctx,slots,
        dtype==COLI_KV_FP8_E4M3?1:dtype==COLI_KV_FP16?2:4,
        dtype==COLI_KV_FP8_E4M3?4:0);
#else
